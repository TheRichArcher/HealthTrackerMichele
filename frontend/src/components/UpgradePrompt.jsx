...
const UpgradePrompt = ({ condition, commonName, isMildCase, requiresUpgrade, confidence, triageLevel, recommendation, onDismiss }) => {
  ...
  const handleSubscriptionClick = () => {
    ...
    if (!isAuthenticated) {
      navigate('/auth?redirect=subscription&action=subscribe');
      return;
    }
  ...
  const handleOneTimeClick = () => {
    ...
    if (!isAuthenticated) {
      navigate('/auth?redirect=one-time-report&action=purchase');
      return;
    }
    
    // Check if user already has access to generate report directly
  ...
  const checkSubscriptionAndGenerateReport = async () => {
    try {
      ...
      if (userTier === 'paid' || userTier === 'one_time') {
        // User already has access, generate report directly
        await generateReport();
      } else {
        // User needs to purchase, redirect to one-time report page
        navigate('/one-time-report');
      }
    } catch (err) {
